...
RSpec.describe Prog::Test::GithubRunner do
  ...
  before do
    ...
    expect(Config).to receive(:e2e_github_installation_id).and_return(123456)
  end
  ...
  describe "#trigger_test_runs" do
    it "triggers test runs" do
      ...
      expect(client).to receive(:workflow_dispatch).with("velioglu/github_e2e", "test_2204.yml", "main").and_return(true)
    ...
    it "can not triggers test runs" do
      client = instance_double(Octokit::Client)
      expect(gr_test).to receive(:client).and_return(client)
      expect(client).to receive(:workflow_dispatch).with("velioglu/github_e2e", "test_2204.yml", "main").and_return(false)
  ...
  describe "#check_test_runs" do
    it "check test runs completed" do
      client = instance_double(Octokit::Client)
      expect(gr_test).to receive(:client).and_return(client)
      expect(client).to receive(:workflow_runs).with("velioglu/github_e2e", "test_2204.yml", {branch: "main"}).and_return({workflow_runs: [{conclusion: "success", created_at: Time.now}]})
      expect(gr_test).to receive(:frame).and_return({"created_at" => Time.new(2023, 1, 1), "test_cases" => ["github_runner_ubuntu_2204"]}).at_least(:once)
    ...
    it "check test runs in progress with nil" do
      client = instance_double(Octokit::Client)
      expect(gr_test).to receive(:client).and_return(client)
      expect(client).to receive(:workflow_runs).with("velioglu/github_e2e", "test_2204.yml", {branch: "main"}).and_return({workflow_runs: [{conclusion: nil, created_at: Time.now}]})
      expect(gr_test).to receive(:frame).and_return({"created_at" => Time.new(2023, 1, 1), "test_cases" => ["github_runner_ubuntu_2204"]}).at_least(:once)
    ...
    it "check test runs in progress state" do
      client = instance_double(Octokit::Client)
      expect(gr_test).to receive(:client).and_return(client)
      expect(client).to receive(:workflow_runs).with("velioglu/github_e2e", "test_2204.yml", {branch: "main"}).and_return({workflow_runs: [{conclusion: "in_progress", created_at: Time.now}]})
      expect(gr_test).to receive(:frame).and_return({"created_at" => Time.new(2023, 1, 1), "test_cases" => ["github_runner_ubuntu_2204"]}).at_least(:once)
      expect { gr_test.check_test_runs }.to nap(15)
    end
    ...
    it "check test runs failed" do
      client = instance_double(Octokit::Client)
      expect(gr_test).to receive(:client).and_return(client)
      expect(client).to receive(:workflow_runs).with("velioglu/github_e2e", "test_2204.yml", {branch: "main"}).and_return({workflow_runs: [{conclusion: "failure", created_at: Time.now}]})
      expect(gr_test).to receive(:frame).and_return({"created_at" => Time.new(2023, 1, 1), "test_cases" => ["github_runner_ubuntu_2204"]}).at_least(:once)
      expect { gr_test.check_test_runs }.to hop("clean_resources")
    end
    ...
    it "check test runs created before" do
      client = instance_double(Octokit::Client)
      expect(gr_test).to receive(:client).and_return(client)
      expect(client).to receive(:workflow_runs).with("velioglu/github_e2e", "test_2204.yml", {branch: "main"}).and_return({workflow_runs: [{conclusion: "failure", created_at: Time.new(2023, 1, 1)}]})
      expect(gr_test).to receive(:frame).and_return({"created_at" => Time.now, "test_cases" => ["github_runner_ubuntu_2204"]}).at_least(:once)
      expect { gr_test.check_test_runs }.to hop("clean_resources")
    end
  end
  ...
  describe "#clean_resources" do
    it "not clean with github exists" do
      ...
      expect(client).to receive(:workflow_runs).with("velioglu/github_e2e", "test_2204.yml", {branch: "main"}).and_return({workflow_runs: [{id: 10}]})
      expect(client).to receive(:cancel_workflow_run).with("velioglu/github_e2e", 10)
    ...
    it "cleans resources and hop finish" do
      ...
      expect(GithubInstallation).to receive(:first).and_return(instance_double(GithubInstallation, repositories: [instance_double(GithubRepository, incr_destroy: nil)]))
      expect(GithubInstallation).to receive(:first).and_return(instance_double(GithubInstallation, destroy: nil))
  ...
  describe "get_client" do
    it "get_client" do
      expect(Github).to receive(:app_client).and_return(Octokit::Client)
