...
module Github
  def self.oauth_client
    Octokit::Client.new(client_id: Config.gh_app_client_id, client_secret: Config.gh_app_client_secret)
  end

  def self.app_client
    current = Time.now.to_i
    private_key = OpenSSL::PKey::RSA.new(Config.gh_app_private_key)
    key = {
      iat: current,
      exp: current + (10 * 60),
